...
- 首页:真实打卡天数横幅(已读/收藏统计)、头条书架轮播、分类筛选、专题封面、资讯流、**关键词搜索**、滑到底部**翻看 30 天历史归档**(日期分隔)
- 快讯:时间轴速递,点击站内打开详情
- 收藏页:收藏的新闻持久保存(含全文,可离线读);我的页:统计 + 来源说明
- 详情页:站内读全文(段落/小标题/配图)、英文原题对照、系统分享面板、**任意位置右滑返回**
- 刷新:下拉/按钮触发,基线数据 + **即时直连源站抓增量**,提示"比上次新增 N 条";已读新闻标题置灰
- 启动本地缓存秒开;**深色模式跟随系统**;按压反馈/切换动效,适配"减弱动态效果"

## 真实新闻数据流

```
GitHub Actions(每 30 分钟,也可手动触发)
  └─ scripts/fetch-news.mjs
       收集:游民星空(列表 AJAX 两页)+ 3DM(列表解析)
             + 机核/游研社/触乐/indienova/IGN/GameSpot(RSS)
